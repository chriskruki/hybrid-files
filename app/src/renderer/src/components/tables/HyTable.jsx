      ...
      <StyledTableRow x={{ '& > *': { borderBottom: 'unset' } }}>
        ...
        {subLabel && (
          <StyledTableCell sx={{ width: '75px' }}>
            <IconButton
              sx={{ color: 'white', disabled: row.groups.length > 0 }}
            ...
            <Collapse in={open} timeout="auto" unmountOnExit>
              <Box sx={{ margin: 1 }}>
                ...
                <Table className="border border-gray-800 rounded" size="small">
                  ...
                  <TableBody>{subBody(row)}</TableBody>
                </Table>
              </Box>
            </Collapse>
          </StyledTableCell>
...
export default function HyTable({ dataList, header, body }) {
  return (
    <TableContainer
      className="border border-gray-800 overflow-auto h-full"
      component={Paper}
      sx={{ backgroundColor: '#455468' }}
    >
      <Table stickyHeader aria-label="collapsible table">
        <TableHead>{header}</TableHead>
        <TableBody>{dataList.length > 0 && body}</TableBody>
      </Table>
    </TableContainer>
  )
}
